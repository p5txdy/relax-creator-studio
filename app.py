...
COMIC_STYLE_PRESETS = (
    ...
    "现代都市条漫，写实动漫，高级电影调色",
    "韩系唯美漫画，精致五官，柔光氛围",
    "现代都市韩漫，精致高颜值人物，修长自然比例，清晰线稿，柔和渐变上色，细腻皮肤质感，戏剧化表情与暧昧氛围光，竖屏网漫构图",
)
...
class StudioApp:
    ...
    def show_comic(self) -> None:
        ...
        self.comic_motion_var = StringVar(value=str(comic.get("motion_mode", "上下交替关键帧")))
    ...
    def _build_comic_character_step(self, parent) -> None:
        ...
        self._field_label(editor, "固定外貌、服装与标志物").pack(anchor="w", pady=(13, 5))
    ...
    def _build_comic_generation_step(self, parent) -> None:
        ...
        self._button(footer, "生成选中分镜", self.generate_selected_comic_shot, kind="primary").pack(side=RIGHT, padx=(0, 8))
    ...
    def _build_comic_video_step(self, parent) -> None:
        ...
        outer.pack(fill=X, padx=(0, 8), pady=(0, 2))
        content = outer.winfo_children()[0]
        self._comic_section_title(content, "06", "静态漫剪映草稿（可选）", "把每张分镜图作为可编辑图片片段写入剪映，并添加上下平移起止关键帧。")
        ...
        Label(motion_card, text="03  静态图片关键帧", bg=SURFACE, fg=INK, font=("Microsoft YaHei UI", 9, "bold")).pack(anchor="w")
        RoundedCombobox(motion_card, textvariable=self.comic_motion_var, values=["上下交替关键帧", "向上移动关键帧", "向下移动关键帧", "无关键帧"], state="readonly").pack(fill=X, pady=(8, 0))
        Label(motion_card, text="仅改变图片纵向位置，不做 AI 动画、人物动作、推近或左右摇移。", bg=SURFACE, fg=MUTED, font=("Microsoft YaHei UI", 8)).pack(anchor="w", pady=(7, 0))
    ...
    def edit_seedream_settings(self) -> None:
        dialog = Toplevel(self.root)
        dialog.title("Doubao Seedream 5.0 Pro 连接设置")
        ...
        Label(dialog, text="使用火山方舟生成角色定妆、场景定景与分镜；每个分镜最多附带 10 张角色和场景参考图。", bg=BG, fg=MUTED, font=("Microsoft YaHei UI", 9)).pack(anchor="w", padx=24)
        ...
        self._field_label(card, "模型 ID").pack(anchor="w", pady=(12, 4))
    ...
    def _sync_comic_state(self) -> None:
        ...
        next_motion = self.comic_motion_var.get().strip() or "上下交替关键帧"
        if next_motion not in {"上下交替关键帧", "向上移动关键帧", "向下移动关键帧", "无关键帧"}:
            next_motion = "上下交替关键帧"
    ...
    def _comic_character_list_text(item: dict[str, object]) -> str:
        ...
        return f" {ready}  {item.get('name', '未命名角色')}  ·  {item.get('status', '未生成')}"
    ...
    def generate_comic_character(self) -> None:
        ...
        prompt = str(character.get("prompt", "")).strip()
        if not prompt:
            prompt = build_character_prompt(str(character.get("name", "角色")), str(character.get("description", "")), self.state["comic"]["art_style"])
        ...
        if prompt != str(character.get("prompt", "")).strip():
            ...
            self.comic_character_prompt_editor.delete("1.0", END)
            self.comic_character_prompt_editor.insert("1.0", prompt)
        if not messagebox.askyesno("生成候选定妆", "将使用 Doubao Seedream 5.0 Pro 生成一张候选定妆图。生成后需要预览并点击“确认候选为参考图”，才会用于后续镜头。是否继续？"):
            return
    ...
    def _generate_comic_shots(self, indices: list[int]) -> None:
        ...
        if too_many_references:
            messagebox.showwarning("参考图过多", "Seedream 5.0 Pro 每次最多接收 10 张参考图（角色与场景合计），请调整以下分镜：\n" + "\n".join(too_many_references))
            return
        if missing:
        ...
        if missing_scenes:
            ...
            return
        if len(indices) > 1 and not messagebox.askyesno("批量生成分镜", f"将使用 Seedream 5.0 Pro 按顺序生成 {len(indices)} 张分镜图，可能产生火山方舟 API 费用。是否继续？"):
            return
        art_style = comic["art_style"]
        aspect = comic["aspect"]
    ...
    def _handle_comic_bus(self, event: str, payload: object) -> None:
        if event == "comic_api_ok":
            ...
            self.comic_status.configure(text=f"正在生成 {position}/{total}：{shot.get('title', '')}", fg=ACCENT_DARK)
            self._update_comic_shot_tree_row(index)
            ...
            shot = self.state["comic"]["shots"][index]
            shot.update({"task_id": str(result.get("id", "")), "image_url": str(result.get("imageUrl", "")), "local_path": local_path, "status": "已完成", "progress": "100%", "error": "", "final_prompt": prompt})
