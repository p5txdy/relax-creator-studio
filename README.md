...
```text
dist-macos/解压创作工坊-v0.2.5.app
dist-macos/解压创作工坊-v0.2.5-macOS.dmg
```
...
4. 所有视频片段的素材原声会被静音，主音频、可选背景音乐和 SRT 字幕进入各自独立轨道。
...
Windows 打包脚本会自动安装 PyInstaller，并在缺少 `vendor` 目录时准备剪映草稿与 MediaInfo 依赖。打包产物位于 `dist\解压创作工坊-v0.2.5.exe`，是可独立运行的单个 EXE。不同版本使用不同文件名，不会覆盖旧版本。
