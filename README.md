...
```text
dist-macos/解压创作工坊-v0.2.4.app
dist-macos/解压创作工坊-v0.2.4-macOS.dmg
```
...
Windows 打包脚本会自动安装 PyInstaller，并在缺少 `vendor` 目录时准备剪映草稿与 MediaInfo 依赖。打包产物位于 `dist\解压创作工坊-v0.2.4.exe`，是可独立运行的单个 EXE。不同版本使用不同文件名，不会覆盖旧版本。
