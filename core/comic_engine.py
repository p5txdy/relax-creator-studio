...
CHARACTER_REFERENCE_CONSTRAINT = (
    "人物参考图硬性要求：画面中只出现该角色一人，独立正面全身定妆，纯白或浅灰无缝背景，"
...
def enforce_character_reference_prompt(prompt: str) -> str:
    ...
    if "人物参考图硬性要求" in base:
        return base
    return f"{base}，{CHARACTER_REFERENCE_CONSTRAINT}"
...
def build_character_prompt(name: str, description: str, art_style: str) -> str:
    details = description.strip() or "请根据人物身份设计鲜明、易辨认且可重复使用的外貌与服装"
    return enforce_character_reference_prompt(
        f"{name}，{details}，{art_style}，单一角色定妆照，正面全身，清晰五官，固定发型与服装，"
        "角色设计稿，高细节，竖版 2:3 构图"
    )
